...
void TrackLinkClient::receiveData()
{
    ...
    while (!connGood && !threadExit)
    {
        ...
        if (!udpman->Create())
        {
			UE_LOG(PharusTracker, Log, TEXT("PharusTracker :: TrackLinkClient :: --- Unable to create socket, retrying... ---"));
		...
		{ 
			...
			UE_LOG(PharusTracker, Log, TEXT("PharusTracker :: TrackLinkClient :: Attempting to bind socket on NIC: %s"), *FString(localIP)); // ID DEBUG ONLY
		}
        else
		{
        ...
        if (!bindOK)
        {
			UE_LOG(PharusTracker, Log, TEXT("PharusTracker :: TrackLinkClient :: --- Unable to bind socket to port! ---"));
			std::this_thread::sleep_for(std::chrono::seconds(1)); // ID HACK
            udpman->Close();
            delete udpman;
            udpman = nullptr;
        ...
        UE_LOG(PharusTracker, Log, TEXT("PharusTracker :: TrackLinkClient :: --- Successfully set up link at port! ---"));
    ...
    while (!threadExit)
    {
        ...
        if (recvSize > 0)
        {
            ...
            while (curPos < recvSize)
            {
                if (recvBuf[curPos++] != 'T')
                {
					UE_LOG(PharusTracker, Warning, TEXT("PharusTracker :: TrackLinkClient :: --- Unexpected header byte, skipping packet! ---"));
                    curPos = recvSize;
                    continue;
                }
                ...
                if (recvBuf[curPos++] != 't')
                {
					UE_LOG(PharusTracker, Warning, TEXT("PharusTracker :: TrackLinkClient :: --- Unexpected tailing byte, skipping packet! ---"));
                    curPos = recvSize;
                    recvMutex.unlock();
                    continue;
                }
