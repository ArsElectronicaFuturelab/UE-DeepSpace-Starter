...
{
	UE_LOG(PharusTracker, Warning, TEXT("APharusTrackerManager :: BeginPlay BUILD %s"), *PharusTrackerBuildNumber); 
	...
	UE_LOG(PharusTracker, Warning, TEXT("APharusTrackerManager :: TrackLink Client :: NIC to Bind: %s"), *FString(c_BindNIC));
	...
	UE_LOG(PharusTracker, Warning, TEXT("APharusTrackerManager :: TrackLink Infrastructure :: --- Receiver Running! ---")); 
}
	...
	if (idsToSpawn.Num() > 0) {
		...
		for (int i = idsToSpawn.Num() - 1; i >= 0; i--) {
			...
			if(_track){ 
				...
				if (bUseLocalSpace) {
					TrackUpdated(_track->trackID, GetActorLocation() + UKismetMathLibrary::Quat_RotateVector(GetActorRotation().Quaternion(), FVector(_track->currentPos.x * SizeX, _track->currentPos.y * SizeY, 0)), UKismetMathLibrary::MakeRotFromYZ(FVector(_track->orientation.x, -_track->currentPos.y, 0), FVector::UpVector));
					PharusTransforms.Add(_track->trackID, FTransform(UKismetMathLibrary::MakeRotFromYZ(FVector(_track->orientation.x, -_track->currentPos.y, 0), FVector::UpVector), GetActorLocation() + UKismetMathLibrary::Quat_RotateVector(GetActorRotation().Quaternion(), FVector(_track->currentPos.x * SizeX, _track->currentPos.y * SizeY, 0)), FVector::OneVector));
				}
				else {
					TrackUpdated(_track->trackID, FVector(_track->currentPos.x * SizeX, _track->currentPos.y * SizeY, 0), UKismetMathLibrary::MakeRotFromYZ(FVector(_track->orientation.x, -_track->currentPos.y, 0), FVector::UpVector));
					PharusTransforms.Add(_track->trackID, FTransform(UKismetMathLibrary::MakeRotFromYZ(FVector(_track->orientation.x, -_track->currentPos.y, 0), FVector::UpVector), GetActorLocation() + FVector(_track->currentPos.x * SizeX, _track->currentPos.y * SizeY, 0), FVector::OneVector));
				}
		...
		for (int i = idsToChange.Num() - 1; i >= 0; i--) {
			...
			if (_track) {
				...
				if (bUseLocalSpace) {
					...
					FVector loc = GetActorLocation() + UKismetMathLibrary::Quat_RotateVector(GetActorRotation().Quaternion(), FVector(_track->currentPos.y * SizeX, _track->currentPos.x * SizeY, 0));
					...
					TrackUpdated(_track->trackID, FVector(_track->currentPos.y * SizeX, _track->currentPos.x * SizeY, 0), UKismetMathLibrary::MakeRotFromYZ(FVector(_track->orientation.y, -_track->currentPos.x, 0), FVector::UpVector));
					PharusTransforms.Add(_track->trackID, FTransform(UKismetMathLibrary::MakeRotFromYZ(FVector(_track->orientation.y, -_track->currentPos.x, 0), FVector::UpVector), GetActorLocation() + FVector(_track->currentPos.y * SizeX, _track->currentPos.x * SizeY, 0), FVector::OneVector));
				}
...
void APharusTrackerManager::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	UE_LOG(PharusTracker, Warning, TEXT("APharusTrackerManager :: TrackLink Infrastructure :: --- Shutdown Complete! ---"));
...
void APharusTrackerManager::SpawnTrackerRepresentant(int trackID) {
	...
	if (world) {
		...
		if (bLogTrackerSpawned) {
			UE_LOG(PharusTracker, Log, TEXT("Tracker %d spawned"), trackID);
		}
	}
}
...
void APharusTrackerManager::RemoveTrackerRepresentant(int trackID) {
	...
	if (bLogTrackerRemoved) {
		UE_LOG(PharusTracker, Log, TEXT("Tracker %d removed"), trackID);
	}
...
void APharusTrackerManager::UpdateTrackerRepresentant(int trackID, FVector2D orientation, FVector2D currentPos) {
	...
	if (actor && (*actor)->GetRootComponent()) {
		if (bUseLocalSpace) {
			FVector loc = GetActorLocation() + UKismetMathLibrary::Quat_RotateVector(GetActorRotation().Quaternion(), FVector(currentPos.X * SizeX, currentPos.Y * SizeY, 0));
			...
			(*actor)->SetActorLocationAndRotation(FVector(currentPos.X * SizeX, currentPos.Y * SizeY, 0), UKismetMathLibrary::MakeRotFromYZ(FVector(orientation.X, -orientation.Y, 0), FVector::UpVector)); 
		}		

		if (bLogTrackerUpdated) {
			UE_LOG(PharusTracker, Log, TEXT("Tracker %d updated"), trackID);
		}
	}
}